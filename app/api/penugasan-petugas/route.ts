...
export async function POST(request: Request) {
  ...
  try {
    ...
    // const response = await prisma.pengaduan.findUnique({ where: { id: pengaduanId }, include: {
    //   petugas: true
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (pengaduanId === null) {
      return NextResponse.json({ ...resultCode(214) })
    }
    ...
    const allPengaduanByPetugas = await prisma.pengaduan.findUnique({
      include: {
        pelanggan: {
          select: {
            id: true,
