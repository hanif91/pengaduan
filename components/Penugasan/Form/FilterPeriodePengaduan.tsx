...
type FilterPeriodePenugasanProps = {
    month: number,
    setMonth: any,
    year: number,
    setYear: any,
...
const FilterPeriodePenugasan = (
    {
        month,
        setMonth,
        year,
        setYear
    }: FilterPeriodePenugasanProps
) => {

    const [yearsPenugasan, setYearsPenugasan] = useState<[]>([]);

    useEffect(() => {
        const fetchYearPenugasan = async() => {
            const getYearPenugasan = await axios.get('/api/get/tahun-penugasan');
            console.log('getYearPenugasan', getYearPenugasan.data.data)
            setYearsPenugasan(getYearPenugasan.data.data)
        }
        fetchYearPenugasan();
    }, [])
        ...
        <Box sx={{ display: "flex", flexDirection: "row", gap: 2}}>
            ...
            <Box>
                <FormControl fullWidth size="medium" sx={{minWidth: '100px'}}>
                    ...
                    >
                        {yearsPenugasan.length > 0 ?
                        yearsPenugasan.map((year, key) => {
...
export default FilterPeriodePenugasan