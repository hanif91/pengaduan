...
type FilterPeriodePengaduanProps = {
    month: number,
    setMonth: any,
    year: number,
    setYear: any,
...
const FilterPeriodePengaduan = (
    {
        month,
        setMonth,
        year,
        setYear
    }: FilterPeriodePengaduanProps
) => {

    const [yearsPengaduan, setYearsPengaduan] = useState<[]>([]);

    useEffect(() => {
        const fetchYearPengaduan = async() => {
            const getYearPengaduan = await axios.get('/api/get/tahun-pengaduan');
            console.log('getYearPengaduan', getYearPengaduan.data.data)
            setYearsPengaduan(getYearPengaduan.data.data)
        }
        fetchYearPengaduan();
    }, [])
        ...
        <Box sx={{ display: "flex", flexDirection: "row", gap: 2}}>
            ...
            <Box>
                <FormControl fullWidth size="medium" sx={{minWidth: '100px'}}>
                    ...
                    >
                        {yearsPengaduan.length > 0 ?
                        yearsPengaduan.map((year, key) => {
...
export default FilterPeriodePengaduan